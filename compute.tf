...
resource "oci_core_instance" "StreamSets_Data_Collector" {
  count = "1"
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[var.AD - 1],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "StreamSets_Data_Collector"
  hostname_label      = "StreamSets_Data_Collector"
  shape               = "${var.DataCollectorShape}"
  subnet_id           = "${oci_core_subnet.private.*.id[var.AD - 1]}"
